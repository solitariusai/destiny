...
fn _get_prefix_id(id: u32) -> String {
...
fn _filename_condition(
    ...
    format!("{filename}-{id}-of-PLACEHOLDER.{extension}")
}
...
pub fn _save_safetensors_fn(
    ...
    for (_key, _arrays) in state_dict.iter(){
        filename_with_extension = _filename_condition(
            filename, shard_idx, Some(extension));
        ...
        // let _tensors: PyTensor = PyTensor::from_pyany(py, &_arrays)?;
        if accm_byte_size > max_shard_byte_size {
            _ = serialize_to_file(
                arrays.iter().map(|(k, v)| {
                    Ok((k, _get_tensor_view(v)?))
